...
-
-## 概要
-開発中のテスト効率化、およびAPIコスト削減やデモ用途のため、生成AIを使用せずに動作確認ができるモード。
-
-## 仕様
-- **有効化条件**:
-  - URLパラメータ `?dev=true` (case-insensitive) を付与してアクセスする。
-  - 一度有効になると `sessionStorage` に状態が保存され、ブラウザを閉じるまで（または別タブで開くまで）維持される。
-- **表示**:
-  - 画面上部（`layout.tsx`）に黄色の警告帯「devモードで実行」を常時表示。
-- **挙動**:
-  - `/api/cook` エンドポイントが `isDev: true` を受け取ると、AI API呼び出しをスキップする。
-  - 即座に固定のモックレスポンス（カレーの画像・テキスト）を返す。