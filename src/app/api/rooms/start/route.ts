...
import { StoreUnavailableError, startGame, getRoom, getStoreDebug, normalizeRoomId } from '@/lib/store';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof StoreUnavailableError) {
      console.error('[room-start]', { status: 'store_unavailable', roomId: roomIdForLog, ...getStoreDebug(), vercelRequestId: request.headers.get('x-vercel-id') });
